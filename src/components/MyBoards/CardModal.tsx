...
function CardModal(props: CardModalProps) {
  ...
  useEffect(() => {
    ...
    if (props.card && props.reminders) {
      let cardReminders = props.reminders.filter(
        (reminder) =>
          reminder.relatedTaskId &&
          reminder.relatedTaskId === props.card.id?.toString()
      );
      setReminders(cardReminders);
    }
  }, [props.card, props.reminders]);
    ...
    props.card !== null && (
      ...
      >
        <CalendarModal
          ...
          relatedTaskId={props.card.id?.toString() ?? undefined}
        />
        ...
        <div className="bg-white dark:bg-slate-950 w-full sm:w-5/6 md:w-2/3 lg:w-1/2 p-4 rounded-lg shadow-md transform transition-transform ease-in-out duration-300 max-h-[80vh]">
          ...
          <span className="flex flex-col w-full max-h-[65vh] overflow-y-auto gap-2 dark:text-white">
            ...
            <span className="flex w-full justify-start items-center gap-2 mt-6">
              ...
              >
                {!(reminders.length > 0) ? "Add Reminder" : "View Calendar"}
              </h2>
            </span>
                ...
                {reminders.map((reminder) => {
                  return (
                    <span key={reminder._id}>
                      <b>Reminder:</b> {reminder.reminderText} <b>Date:</b>{" "}
